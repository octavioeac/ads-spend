...
# Existing Service Account (replace with yours)
# e.g. "123456789012-compute@developer.gserviceaccount.com"
...
if gcloud storage buckets describe "gs://${BUCKET_MODELOS}" --project="$PROJECT_ID" >/dev/null 2>&1; then
  ...
  echo "Creating bucket gs://${BUCKET_MODELOS} ..."
...
# 3) Grant minimum roles to the Service Account
# BigQuery: read data and run jobs
...
echo "Roles applied to ${SA_EMAIL} "
...
# 8) Deploy function (public: allow unauthenticated)
...
echo
echo "== Test with curl (no auth needed because of --allow-unauthenticated) =="
